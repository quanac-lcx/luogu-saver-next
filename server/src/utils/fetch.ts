...
function handleRequestError(err: any): never {
    ...
    if (code === 'ECONNABORTED' || code === 'ETIMEDOUT' ||
        code === 'ECONNREFUSED' || code === 'ECONNRESET' || msg.includes('timeout')
    ) {
...
function mergeSetCookieToHeaders(response: AxiosResponse, headers: Record<string, string>) {
    ...
    const existingCookies = headers['Cookie']
        ? headers['Cookie'].split('; ').reduce((acc, cur) => {
...
export async function fetch(
    ...
    const headers: Record<string, string> = {
        ...
        ...(cookie ? { 'Cookie': Object.entries(cookie).map(([k, v]) => `${k}=${v}`).join('; ') } : {})
    };
    ...
    if (resp.status === 401) {
        logger.warn({ url, cookie: headers['Cookie'] }, "Can't fetch the page. Is the cookie expired? Received 401 Unauthorized.");
